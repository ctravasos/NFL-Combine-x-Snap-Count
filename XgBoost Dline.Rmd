...
dline_data <- dataset_unique %>%
    filter(Pos.x %in% c("DT", "DE"))
```
...
dline_data$heights_in_inches <- sapply(dline_data$Ht, convert_to_inches)



set.seed(12345)
train_index <- sample(1:nrow(dline_data), 0.8 * nrow(dline_data))
train_data <- dline_data[train_index, ]
test_data <- dline_data[-train_index, ]
...
Best iteration is 47, set to 100 with early stopping 20
...
Min child weight 3, max depth 3
```{r}
res_db
```
